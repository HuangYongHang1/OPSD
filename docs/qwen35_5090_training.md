...
| `TEACHER_GUIDANCE_MODE` | `quality` | teacher 使用参考答案的方式。`exact` 要求贴近标准答案原文；`quality` 把标准答案当私有语义参考，更偏向忠实、简洁、自然、可读、有编辑感的摘要。 |
...
默认 `TEACHER_GUIDANCE_MODE=quality` 时，teacher 不要求 student 一字不差复制 `output`，而是偏向忠实、简洁、自然、可读、有编辑感的摘要。它会更明确地偏好抓主线、保留关键事实、避免机械覆盖所有 bullet、避免生硬堆从句；如果要复现实验中的严格 target-answer 版本，可以设置 `TEACHER_GUIDANCE_MODE=exact`。
